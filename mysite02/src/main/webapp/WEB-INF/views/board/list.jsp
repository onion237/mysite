			...
			<div id="board">
				...
				<div class="pager">
					<ul>
						...
						<c:if test="${pager.prevPage != -1 }">
							<li><a href="${pageContext.request.contextPath }/board?a=list&cur=${pager.prevPage}&keyword=${param.keyword}">◀</a></li>
						</c:if>
						...
						<c:forEach begin="${pager.begin }" end="${pager.end }" step="1" varStatus="status">
							<c:choose>
								<c:when test="${status.index <= pager.totalPage }">
									...
									<c:if test="${status.index != pager.curPage}">
										<li><a href="${pageContext.request.contextPath }/board?a=list&cur=${status.index}&keyword=${param.keyword}">${status.index}</a></li>
									</c:if>								
								</c:when>
						...
						<c:if test="${pager.nextPage != -1 }">
							<li><a href="${pageContext.request.contextPath }/board?a=list&cur=${pager.nextPage }&keyword=${param.keyword}">▶</a></li>
						</c:if>
						<c:if test="${pager.nextPage == -1 }">
							<li>▶</li>
						</c:if>
				...
				<div class="bottom">
					<c:if test="${user != null}">
						<a href="${pageContext.request.contextPath }/board?a=writeform&cur=${pager.curPage }&keyword=${param.keyword}"
							id="new-book">글쓰기</a>
					</c:if>
				</div>				
			</div>
